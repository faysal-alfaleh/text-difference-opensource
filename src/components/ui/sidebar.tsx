  ...
  React.useEffect(() => {
    const handleKeyDown = (event: KeyboardEvent) => {
      if (
        ...
        (event.target.isContentEditable || event.target.matches("input, textarea"))
      ) {
        return
      }
      ...
      <Sheet open={openMobile} onOpenChange={setOpenMobile} {...props}>
        ...
        >
          <SheetHeader className="sr-only">
            ...
            <SheetDescription>{content.sidebar.description}</SheetDescription>
          </SheetHeader>
...
function SidebarTrigger({
  ...
  return (
    ...
    >
      <PanelLeftIcon />
      <span className="sr-only">{content.sidebar.toggle}</span>
    </Button>
  )
}
...
function SidebarRail({ className, ...props }: React.ComponentProps<"button">) {
  ...
  return (
    <button
      ...
      aria-label={content.sidebar.toggle}
      tabIndex={-1}
      onClick={toggleSidebar}
      title={content.sidebar.toggle}
      className={cn(
